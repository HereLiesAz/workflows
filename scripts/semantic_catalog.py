...
PURPOSE_PROFILES: Final[dict[str, dict[str, object]]] = {
    ...
    "a95687c1a4f8af0ad53e5d420d557fc7237dd2f97d70d63201ac4cdff6edb6c6": {"signing":"raw-jks","signing_env_style":"release-store","java_version":"21","setup_android":True,"android_packages":"platform-tools","google_services":"none","package_name":"com.hereliesaz.illumera","pre_build_command":"for aar in lib-exoplayer-release.aar lib-decoder-av1-release.aar lib-decoder-ffmpeg-release.aar lib-decoder-iamf-release.aar lib-decoder-mpegh-release.aar; do test -s \"playbackcore/libs/$aar\" || { echo \"Missing committed Stremio playback artifact: $aar\" >&2; exit 1; }; done; BASE=$(grep -oP 'versionName\\s*=.*?:\\s*\"\\K[^\"]+' app/build.gradle.kts | head -1); test -n \"$BASE\" || { echo \"Could not resolve base versionName\" >&2; exit 1; }; MAJOR_MINOR=$(printf '%s\\n' \"$BASE\" | grep -oP '^\\d+\\.\\d+'); test -n \"$MAJOR_MINOR\" || { echo \"Invalid base versionName: $BASE\" >&2; exit 1; }; RELEASE_VERSION_NAME=\"${MAJOR_MINOR}.${TARGET_RUN_NUMBER}\"; RELEASE_VERSION_CODE=\"$((TARGET_RUN_NUMBER * 100 + VERSION_BUILD))\"; echo \"RELEASE_VERSION_NAME=$RELEASE_VERSION_NAME\" >> \"$GITHUB_ENV\"; echo \"RELEASE_VERSION_CODE=$RELEASE_VERSION_CODE\" >> \"$GITHUB_ENV\"","build_command":"./gradlew :app:testDebugUnitTest :app:lintRelease :app:assembleRelease :app:bundlePlay --no-daemon --stacktrace -PversionNameOverride=\"$RELEASE_VERSION_NAME\" -PversionCodeOverride=\"$RELEASE_VERSION_CODE\"","aab_glob":"app/build/outputs/bundle/play/*.aab","apk_glob":"app/build/outputs/apk/release/*.apk","publish_on_push":True,"publish_default":True,"persist_version":False,"tracks":[{"track":"internal","status":"completed"},{"track":"alpha","status":"draft","preserve_existing":True,"name":"illumera $RELEASE_VERSION_NAME"}],"github_built_release_always":True,"github_built_release_tag_env":"RELEASE_VERSION_NAME","github_built_release_include_aab":False,"github_built_release_checksum":True,"github_built_release_changes":True,"release_app_name":"illumera"},
