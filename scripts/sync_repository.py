...
def ensure_checkout_target(job: dict[str, Any], target_private: bool) -> None:
    steps = job.get("steps")
    if not isinstance(steps, list):
        return
    for step in steps:
        ...
        if target_private:
            ...
            with_map.setdefault("token", "${{ github.token }}")
...
def compile_central(source_text: str, target: dict[str, Any], source_path: str, check_name: str) -> str:
    ...
    doc = CommentedMap()
    doc["name"] = f"{source_name} [central]"
...
[[ "$TARGET_REPOSITORY_OWNER_ID" == "103241502" ]] || { echo "OIDC owner ID mismatch" >&2; exit 1; }''',
            },
            {
                ...
                "env": {
                    "GH_TOKEN": "${{ secrets.GH_TOKEN }}", "CHECK_NAME": check_name,
                    "DETAILS_URL": "${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }}",
                },
                "shell": "bash",
                "run": '''set -euo pipefail
payload="$(jq -n --arg name "$CHECK_NAME" --arg sha "$TARGET_CHECK_SHA" --arg details "$DETAILS_URL" '{name:$name,head_sha:$sha,status:"in_progress",details_url:$details,output:{title:$name,summary:"Running centrally in HereLiesAz/workflows."}}')"
...
echo "check_id=$(jq -r '.id' <<<"$response")" >> "$GITHUB_OUTPUT"''',
    ...
    jobs["__central_check_finish"] = CommentedMap({
        ...
        "steps": [{
            ...
            "run": '''set -euo pipefail
[[ -n "${CHECK_ID:-}" ]] || exit 0
if [[ "$FAILED" == "true" ]]; then conclusion="failure"; summary="Central workflow failed. Open the linked run for details."; else conclusion="success"; summary="Central workflow completed successfully."; fi
...
gh api --method PATCH "repos/${TARGET_REPOSITORY}/check-runs/${CHECK_ID}" --input - <<<"$payload"''',
    ...
    doc["jobs"] = jobs
    return "# Generated by HereLiesAz/workflows. Do not edit by hand.\n" + f"# source: {target['full_name']}:{source_path}\n" + dump_yaml(doc)
...
def sync_repository(gh: GitHub, full_name: str, worker_url: str, dry_run: bool = False) -> dict[str, Any]:
    ...
    for item in listing:
        ...
        if item.get("type") != "file" or not path.endswith((".yml", ".yaml")):
            continue
        source_text, _ = gh.get_file(repo["full_name"], path)
        if source_text.startswith(PROXY_MARKER):
            state = workflows_manifest.get(path)
            results.append({"path": path, "status": "proxy", "central_workflow": state.get("central_workflow") if state else None})
            continue
        ...
        try:
            ...
            blockers = [str(exc)]
        if blockers:
            workflows_manifest[path] = {"status": "blocked", "source_sha256": source_hash, "name": workflow_name, "blockers": blockers, "updated_at": now_iso()}
            results.append({"path": path, "status": "blocked", "blockers": blockers})
            continue

        slug = slugify(path)
        registry_source = f"registry/{repo_id}/{slug}.source.yml"
        central_workflow = f".github/workflows/absorbed-{repo_id}-{slug}.yml"
        try:
            compiled = compile_central(dump_yaml(expanded), repo, path, workflow_name)
            proxy = build_proxy(source_text, path, source_hash, workflow_name)
        except Exception as exc:
            workflows_manifest[path] = {"status": "blocked", "source_sha256": source_hash, "name": workflow_name, "blockers": [str(exc)], "updated_at": now_iso()}
            results.append({"path": path, "status": "blocked", "blockers": [str(exc)]})
            continue

        workflows_manifest[path] = {"status": "active", "name": workflow_name, "source_sha256": source_hash, "registry_source": registry_source, "central_workflow": central_workflow, "updated_at": now_iso()}
        if not dry_run:
            gh.put_file(CENTRAL_REPOSITORY, registry_source, source_text, f"Absorb {repo['full_name']}:{path}", branch="main")
            gh.put_file(CENTRAL_REPOSITORY, central_workflow, compiled, f"Compile {repo['full_name']}:{path}", branch="main")
            gh.put_file(repo["full_name"], path, proxy, f"Centralize {path} via {CENTRAL_REPOSITORY}", branch=default_branch)
        results.append({"path": path, "status": "migrated", "central_workflow": central_workflow, "source_sha256": source_hash})
